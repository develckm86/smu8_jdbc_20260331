...
//접속 객체를 반환하는 함수 생성 (static, 객체를 처음에 만들고 공유함)
    ...
    //일반적인 객체(10,true,dto...)는 static 으로 사용해도 상관 없지만
    //접속 객체는 상태가 계속 변하기 때문에 static 으로 적합하지 않다!!
    //접속 객체는 private 으로 막고 getConnection() 함수를 만들어서 새로 접속을 맺어서 반환 (캡슐화)
    /////////////////////////////사용권장하지 않는 코드 //////////////////////////////////
    public static Connection conn;
    static { //생성자 처럼 jvm 실행시 static(클래스맴버)를 처리하는 블럭
        try {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(b); //JVM 이 시작할때 데이터로 만들어서 사용가능
        System.out.println(new L20StaticDBManager().a);

        try (Connection conn=L20StaticDBManager.conn;
