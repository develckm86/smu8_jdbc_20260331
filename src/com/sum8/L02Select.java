...
public class L02Select {
    public static void main(String[] args) throws Exception {
        ...
        Connection conn= conn=DriverManager.getConnection(url,user,pw);; //데이터소스

        Statement stmt=stmt=conn.createStatement();; //sql 실행객체
        //Statement 접속객체가 반환(준다)하는 쿼리 실행객체
        ResultSet rs= rs=stmt.executeQuery(sql); //select,  executeUpdate(dml)
