...
//LOC Varchar(13)
    ...
    //set 을 호출할때는 대부분 저장 수정 삭제 시 사용
    // 유효성 검사 (데이터가 유효한지 검사,무결성 검사) -> 캡슐화를 해서 가능 (Beans 클래스에서 사용)
    /*
    private void setDeptno(int deptno) throws IllegalArgumentException{
//        if(deptno>99) return;
        if(deptno>99) throw new IllegalArgumentException("부서 번호는 100 미만입니다.");
        this.deptno=deptno;
    }
     */
